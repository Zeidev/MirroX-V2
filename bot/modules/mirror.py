...
class MirrorListener(listeners.MirrorListeners):
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ):
        ...
        with download_dict_lock:
            ...
            del download_dict[self.uid]
            count = len(download_dict)
        funame = f'<a href="tg://user?id={self.message.from_user.id}">{self.message.from_user.first_name}</a>'
        fwdpm = f'\n\n<b>Hei {funame}, Your task successfully completed! click button below to see a log channel.</b>'
        logmsg = sendLog(msg + msc, self.bot, self.update, InlineKeyboardMarkup(buttons.build_menu(2)))
